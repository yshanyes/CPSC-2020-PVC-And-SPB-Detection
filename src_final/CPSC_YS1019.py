    ...
    def __data_generation(self, list_inds):
        ...
        for i in range(X.shape[0]):
            ...
            if self.train:
                ...
                rdm = np.random.randn()
                if  rdm > 0.2:
                    rdm1 = np.random.randn()
                    if rdm1 > 0.5: 
                        sig = sig+self.noise['bw'][ind]*0.5
                    elif rdm1 > -0.5: 
                        sig = sig+self.noise['em'][ind]*0.2
                    else: 
                        sig = sig+self.noise['ma'][ind]*0.2
                elif rdm > -0.5:
                    rdm2 = np.random.randn()
                    if rdm2 > 0.5: 
                        sig = scaling(sig)
                    elif rdm2 > -0.5: 
                        sig = shift(sig)
                        ...
                        sig = wavelet_db6(sig)
                    # elif rdm2 > 0.3: sig = wavelet_sym(sig)
                    # elif rdm2 > 0.3: sig = wavelet_db4(sig)
                else:
                    pass
