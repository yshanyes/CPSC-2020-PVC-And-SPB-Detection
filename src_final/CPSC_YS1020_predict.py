...
# reducelr = ReduceLROnPlateau(monitor='val_mean_iou', factor=0.5, verbose=1,mode='max',
...
#     fname_model_weigths = args.model
#     model.load_weights(fname_model_weigths)
# elif args.mtype == 'nest':
#     model = NestUnet(3)
#     fname_model_weigths = args.model
#     model.load_weights(fname_model_weigths)
# elif args.mtype == 'ysnet':
#     import net
#     model = net.build_model(start_neurons=32, dropout_ratio=0, filter_size=1, nClasses=3)
#     fname_model_weigths = args.model
#     model.load_weights(fname_model_weigths)
...
#     fname_model_weigths = args.model #'unetplus_deepsupervision_model_weights_2020-10-19_epoch10.h5'
#     model.load_weights(fname_model_weigths)
...
if model_type == "resnet":
    model = resnet.build_model(block=model_type,start_neurons=16, DropoutRatio=0.5, filter_size=32, nClasses=3)
if model_type == "resnext":
    print(model_type)
    model = resnext.build_model(block=model_type,start_neurons=16, DropoutRatio=0.5, filter_size=15, nClasses=3)
if model_type == "ysnet":
    model = net.build_model(start_neurons=32, dropout_ratio=0, filter_size=1, nClasses=3)
...
#               metrics=['accuracy', 'mse', mean_iou, auc, f1])  # mse、categorical_crossentropy
...
#     model.load_weights(fname_model_weigths)
    ...
    for i in range(0,len(y_pred)):
        if (y_pred[i]>1):
            ...
            if (s_count>=200*0.1):#0.1
                ind = i- round(s_count*0.3)
                s_results.append(ind)

            if (v_count>=200*0.1):#0.1
...
def load_ans():
    ...
    ref_files = list(filter(is_mat, os.listdir(REF_PATH)))
    ...
    # data_files = data_files[:7] + data_files[-1:]
    # ref_files  = ref_files[:7] + ref_files[-1:]
    data_files = data_files[7:9]
    ref_files  = ref_files[7:9] 
    print(data_files)
